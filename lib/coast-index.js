...
function createChartResourceCoastIndex (options = {}) {
  ...
  return {
    ...
    findNearest: async (position, findOptions = {}) => {
      ...
      for (const tile of candidateTiles) {
        const segments = await loadChartResourceTileSegments({ resource, fetchImpl, cache, layerName, tile })
  ...
  async function getChartResource () {
    ...
    if (!resourcePromise) {
      resourcePromise = loadChartResource({ resourceId, signalKBaseUrl, fetchImpl, layerName, zoom })
        .then((loadedResource) => {
...
async function loadChartResource ({ resourceId, signalKBaseUrl, fetchImpl, layerName, zoom }) {
  const resourceUrl = new URL(`/signalk/v2/api/resources/charts/${encodeURIComponent(resourceId)}`, signalKBaseUrl)
  const response = await fetchImpl(resourceUrl.toString())
  if (!response || response.status === 404) {
    throw new Error(`Chart resource '${resourceId}' was not found`)
  }
  if (!response.ok) {
    throw new Error(`Chart resource '${resourceId}' returned HTTP ${response.status}`)
  }
...
async function loadChartResourceTileSegments ({ resource, fetchImpl, cache, layerName, tile }) {
  ...
  if (!response.ok) {
    throw new Error(`Chart tile ${key} returned HTTP ${response.status}`)
  }
