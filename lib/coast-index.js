...
async function loadChartResource ({ resourceId, signalKBaseUrl, fetchImpl, fetchOptions, layerName, zoom }) {
  ...
  if (!response.ok) {
    throw new Error(chartHttpError(`Chart resource '${resourceId}'`, response.status))
  }
...
async function loadChartResourceTileSegments ({ resource, fetchImpl, fetchOptions, cache, layerName, tile }) {
  ...
  if (!response.ok) {
    throw new Error(chartHttpError(`Chart tile ${key}`, response.status))
  }
...
function chartHttpError (subject, status) {
  if (status === 401) {
    return `${subject} returned HTTP 401. Configure signalKAccessToken or SIGNALK_DISTANCE_TO_SHORE_TOKEN when chart resources require authentication.`
  }
  return `${subject} returned HTTP ${status}`
}
