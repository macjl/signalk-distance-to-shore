...
function createAccessRequestManager (options = {}) {
  ...
  async function pollAccessRequest () {
    ...
    try {
      ...
      // If the stored request no longer exists (e.g. server restarted), clear the
      // stale href and create a fresh request rather than failing permanently.
...
// A stale-request error occurs when the server can no longer find the pending request
// (e.g. after a server restart). We detect it by a 404, or a 5xx whose message contains
// "not found" (the server returns 500 with message "not found" in some versions).
function isStaleRequestError (error) {
  if (!error) return false
  if (error.statusCode === 404) return true
  if (error.statusCode >= 500 && typeof error.message === 'string' &&
      error.message.toLowerCase().includes('not found')) return true
  return false
}
