...
// Minimum distance (meters) from a position to the nearest point inside a bbox [minLon, minLat, maxLon, maxLat].
// Returns 0 if the position is inside the bbox.
function minDistanceToBboxMeters (position, bbox) {
  const clampedLon = clamp(position.longitude, bbox[0], bbox[2])
  const clampedLat = clamp(position.latitude, bbox[1], bbox[3])
  if (clampedLon === position.longitude && clampedLat === position.latitude) return 0
  const proj = project({ latitude: clampedLat, longitude: clampedLon }, position)
  return Math.hypot(proj.x, proj.y)
}
